	...
	bool leftHanded)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		ctx->hemisphere.fbTexture[i]		= bgfx::createTexture2D(ctx->hemisphere.fbTextureWidth[i], ctx->hemisphere.fbTextureHeight[i], false, 1, bgfx::TextureFormat::RGBA32F,
		...
		bgfx::TextureHandle fbTextures[]	=
		{
			...
			bgfx::createTexture2D(ctx->hemisphere.fbTextureWidth[i], ctx->hemisphere.fbTextureHeight[i], false, 1, bgfx::TextureFormat::D24, BGFX_TEXTURE_RT)
		};
	...
	ctx->hemisphere.fbReadTexture	= bgfx::createTexture2D(ctx->hemisphere.fbHemiCountX, ctx->hemisphere.fbHemiCountY, false, 1, bgfx::TextureFormat::RGBA32F,
...
void lmSetHemisphereWeights(lm_context *ctx, lm_weight_func f, void *userdata)
{
	...
	ctx->hemisphere.firstPass.weightsTexture = bgfx::createTexture2D(3 * ctx->hemisphere.size, ctx->hemisphere.size, false, 1, bgfx::TextureFormat::RG32F, BGFX_TEXTURE_NONE, mem);
	LM_FREE(weights);
}
